...
%%%%%%homework 2 using Runge Kuta Order 2 Method, graph the 
%%%%%%approximation, then verify second order error.
%%%%%%It will be accompanied by script for Euler(explicit) and RK4.
%--------------------------------------------------------------------------
%calculate U values
...
for n=1:1:m               %%%%Recursive formula for Euler forward(explicit)
    f(n)=exp(t(n)-u(n));
    f2(n)=exp((t(n)+h)-(u(n)+h*f(n)));
    u(n+1)=u(n)+(h*(f(n)+f2(n)))/2;
end
...
for n=1:1:mesh-1              %%%%Recursive formula for Euler forward(explicit)
    ...
    f2dep(n)=exp((tdep(n)+hdep)-(U(n)+hdep*fdep(n)));
    U(n+1)=U(n)+(hdep*(fdep(n)+f2dep(n)))/2;
   end
